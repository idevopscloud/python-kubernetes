...
class Api(object):
    ...
    def __init__(self,
        ...
        if self.base_url.find('https') == 0:
            ...
            if not self._check_file_readable([cert_path]):
                raise KubernetesError("The given credentials files cannot be loaded")
    ...
    def GetPodsJson(self, namespace=None, selector={}):
        '''List all pods on this cluster'''
    ...
    def GetServiceJson(self, name, namespace='default'):
        '''List the specific service on this cluster'''
    ...
    def GetServicesJson(self, namespace=None):
        # Make and send requests
        if namespace:
            url = ('%(base_url)s/namespaces/%(ns)s/services' %
                {"base_url":self.base_url, "ns":namespace})
        ...
        for pod in pod_list.Items:
            for node in node_list:
                if node.name == pod.NodeName:
                    ...
                    node.mem_request_used += pod.MemRequest

        return node_list

    def GetNodeJson(self, node_name):
        url = '{}/nodes/{}'.format(self.base_url, node_name)
        json = self._RequestUrl(url, 'GET')
        data = self._ParseAndCheckKubernetes(json.content)
        return data
    ...
    def GetEvents(self, namespace='default', involved_obj_kind = None, involved_obj_name = None):
        ...
        if len(selector):
            url = self._BuildUrl(url, extra_params={'fieldSelector': self._gen_selector_str(selector)})
        json = self._RequestUrl(url, 'GET')
        if json.status_code not in [200, 404]:
            raise KubernetesError(json.json()['message'], json.status_code)
        elif json.status_code == 404:
    ...
    def GetEventsJson(self, namespace='default', involved_obj_kind=None, involved_obj_name=None):
        url = '{}/namespaces/{}/events'.format(self.base_url, namespace)
